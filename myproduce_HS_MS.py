...
def produce_HS_nir_bis(M, A, tabwave, fname, dname, snr=50, sigma=0, d=0.31):
    ...
    for i in range(lh):
        ...
        Hifft = np.fft.ifftshift(np.fft.ifft2(H,norm = 'ortho'))             #A Supprimer
        ...
        Hreshaped = np.fft.fft2(np.fft.fftshift(Hreshaped),norm = 'ortho')
...
def mult_noise(Y):
    ...
    print('Apply multiplicative noise done') #Asupprimer
    return noise
...
def add_noise_corr_ns(Y, sigma2=SIG2READ_NS, nframes=NFRAMES_NS):
    ...
    print('Apply correlated ns noise done') #Asupprimer
    return noisecorr
...
def add_noise_corr_nc(Y, sigma2=SIG2READ_NC, nframes=NFRAMES_NC):
    ...
    print('Apply correlated nc noise done') #Asupprimer
    return noisecorr
...
####ORIGINAL#### def main(file_data='Data/', file_save='Data/', snr=50, sigma=0):
...
def main(file_data, file_save, snr=50, sigma=0): #Modifié
    ...
    Yh_highsnr = Y + add_noise_nocorr(Y,sigma2=SIG2READ_NS)#Modifié
...
#####COMMENTAIRES######
'''
compute_symmpad(Matrice,fact_pad) ajoute fact_pad lignes et colonne à la Matrice
import matplotlib.pyplot as plt
Hcentre = np.abs(np.fft.fftshift(H))
NpixR et NpixC je les ai choisies car je sais que X apres compute_sympad aura une taille 174x984
Comme la psf est symmétrique alors je me suis référée à la dim la plus faible cad la ligne de X (174)
donc j'ai pris 174x174 de la psf '

-------------------------------------------------------------------------------
si je veux prendre en compte le ss éche (sumsabmple2d) alors je décommente Y = np.zeros((lh, p_, q_)) car:
    ...
    X = 90x900 --> fact_pad =84 --> X = 174x984
    
    !!!!! ICI H de base il est 384x384 je construit une nvle PSF de 174x984
    
    Tempo = 172x984 prcq IFFT2(H*X)[:-2,:-2]
    Tempo = 90 x 900 prcq _centered(Tempo,(90,900)) centered c'est inverse de compute_symmpad'
    
    Y = 28x279 prcq subsample2d(Tempo,d = 0.31)
    
    #Y[i] = subsample2d(np.real(_centered(np.fft.ifft2(H*np.fft.fft2(X, norm='ortho'), norm='ortho')[:-2, :-2], (p, q))))
-------------------------------------------------------------------------------
!!!!!!!!!!! pour que compute_symmpad fonctionne il faut que dim(X)>fact_pad
'''
