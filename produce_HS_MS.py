...
def produce_HS_nir_bis(M, A, tabwave, fname, dname, snr=50, sigma=0, d=0.31):
    ...
    '''---------------------------------------'''  
    #Asupprimer(en prennand en compte que X est 174x984)
    NbpixR = 174
    NbpixC = 174
    Nshift = (984 - 174)//2
    flag = 1;
    ...
    for i in range(lh):
        ...
        CLbnd = ind[1] - NbpixR//2
        CUbnd = ind[1] + NbpixR//2
        
        #PSF reshaped
        Hreshaped = 0 * Hreshaped
        Hreshaped[:,Nshift:(Nshift+NbpixC)] = Hshift[RLbnd:RUbnd,CLbnd:CUbnd]
        Hreshaped = np.fft.fftshift(Hreshaped)
          
        #Convolve with PSF without subsample
        Y[i] = np.real(np.fft.ifft2(Hreshaped*np.fft.fft2(X, norm='ortho'), norm='ortho'))
        ...
        plt.imshow(np.abs(np.fft.fftshift(Hreshaped)))
        plt.show()
        ...
        #C = np.fft.ifft2(H*np.fft.fft2(X, norm='ortho'), norm='ortho') #A supprimer
...
def main(file_data, file_save, snr=50, sigma=0):
